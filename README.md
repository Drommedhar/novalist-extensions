...
| --- | --- | --- | --- |
| **Formats** | `com.novalist.formats` | `formats-v*` | Exports to HTML, RTF, ODT, plain text, Fountain and FictionBook 2, each marked with the language you write in and carrying your cover where the format can hold one, plus CSV, JSON and OPML for pulling the outline into a spreadsheet, a script or an outliner. Imports Scrivener projects, Ulysses sheets, folders of Markdown, and CSV/TSV files. Checks a finished EPUB before you send it. |
| **Insight** | `com.novalist.insight` | `insight-v*` | Read-only reports over the whole manuscript: name drift and consistency, project health (orphan entries, dangling links, unused images), a continuity worklist when a Codex entry changes, a word-frequency concordance, a pacing curve, and a cast presence report showing who drops out of the book and for how long. Also reports what is in the manuscript: chapters, scenes, words, characters and a printed-page estimate.|
| **Toolkit** | `com.novalist.toolkit` | `toolkit-v*` | Writing sprints and a Pomodoro timer, a task board over your to-do comments, dictionary and thesaurus lookup on a selected word, an offline name generator with per-culture data, and web page capture that keeps the readable text rather than just the title. |
| **Speech** | `com.novalist.speech` | `speech-v*` | The speech engine behind the Narration view: designs a voice from a character's description, and performs a line in that voice with the emotion supplied separately, so one character sounds like themselves whether they are furious or wrecked. Runs entirely on the writer's machine, on the graphics card where there is one, and builds its own Python environment on first use. |
